...
func InjectedEmployeePayrollDetailuseCase(ctx context.Context) employee_payroll_detail.UseCaser {
	configConfig := config.ParseConfig()
	db := config.NewDB(configConfig)
	employeePayrollDetailDetailRepository := mysql.NewEmployeePayrollDetailDetailRepository(db)
	useCaser := employee_payroll_detail.NewUseCase(employeePayrollDetailDetailRepository)
	return useCaser
}
